...
QVariant EpisodeModel::data(const QModelIndex &index, int role) const
{
    ...
    case Qt::DisplayRole :
    {
        ...
        case EpisodeCreationDateTime:  sqlColumn = Constants::EPISODES_DATETIMEOFCREATION; break;
    ...
    {
        // TODO: create a cache?
        QDate createDate = d->_sqlModel->data(d->_sqlModel->index(index.row(), Constants::EPISODES_DATETIMEOFCREATION)).toDate();
...
QVariant EpisodeModel::headerData(int section, Qt::Orientation orientation, int role) const
{
    ...
    case EpisodeCreationDateTime: return tkTr(Trans::Constants::CREATION_DATE_TIME);
...
void EpisodeModel::populateNewRowWithDefault(int row, QSqlRecord &record)
{
    ...
    record.setValue(Constants::EPISODES_DATETIMEOFCREATION, QDateTime::currentDateTime());
